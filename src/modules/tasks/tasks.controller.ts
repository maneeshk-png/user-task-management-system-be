...
export class TasksController {
  ...
  @Post()
  create(@Body()  createTaskDto:CreateTaskDto | CreateTaskDto[],@CurrentUser() user:User ) {
    this.logger.log(`req.user: ${JSON.stringify(user)}`);

    //create Tasks for authenticated user
    if(Array.isArray(createTaskDto)){
      return this.tasksService.createManyTask(user.id,createTaskDto);
    }
